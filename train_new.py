...
# --- helper: normalized STFT‐magnitude loss -----------------------------------
def spectral_loss(y_true, y_pred, n_fft=1024, hop_length=256):
    """
    Spectral convergence + log‐mag L1, normalized by the number of freq‐time bins.
    """
    window = torch.hann_window(n_fft).to(y_true.device)

    Y  = torch.stft(
        y_true.squeeze(1), n_fft=n_fft, hop_length=hop_length,
        window=window, return_complex=True
    )
    Yp = torch.stft(
        y_pred.squeeze(1), n_fft=n_fft, hop_length=hop_length,
        window=window, return_complex=True
    )
    mag, magp = torch.abs(Y), torch.abs(Yp)

    # spectral convergence
    sc = torch.norm(magp - mag, p='fro') / (torch.norm(mag, p='fro') + 1e-8)
    # log‐mag L1
    lm = F.l1_loss(torch.log(mag + 1e-7), torch.log(magp + 1e-7), reduction='mean')

    # normalize by F * T per example
    num_bins = mag.shape[1] * mag.shape[2]
    return (sc + lm) / num_bins
...
def train_model():
    ...
    num_epochs  = 90
    lr          = 1e-4
    lambda_spec = 0.01   # reduced spectral weight
    ...
    # initialize model, optimizer, scheduler, criteria
